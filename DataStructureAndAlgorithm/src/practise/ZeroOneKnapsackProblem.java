...
public class ZeroOneKnapsackProblem {
	...
	static int maxamizeProfit_DP(int[] profit, int[] weight, int capacity) {
		int currentIndex = 0;
		int calculatedProfit[][] = new int[profit.length][capacity + 1];
		return maxamizeProfit_DP_TD(profit, weight, capacity, currentIndex, calculatedProfit);

	}

	static int maxamizeProfit_DP_TD(int[] profit, int[] weight, int capacity, int currentIndex,
			int[][] calculatedProfit) {

		if (currentIndex < 0 || capacity <= 0 || currentIndex >= profit.length)
			return 0;
		int profit1 = 0;
		if (calculatedProfit[currentIndex][capacity] == 0) {

			if (weight[currentIndex] <= capacity)
				profit1 = profit[currentIndex]
						+ maxamizeProfit_DC(profit, weight, capacity - weight[currentIndex], currentIndex + 1); // taking
																												// 1st
																												// element

			int profit2 = maxamizeProfit_DC(profit, weight, capacity, currentIndex + 1); // leaving first element
			calculatedProfit[currentIndex][capacity] = Math.max(profit1, profit2);
		}
		return calculatedProfit[currentIndex][capacity];
	}
